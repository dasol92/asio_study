  ...
  private:
        ...
        if (error) {
            ...
            while (nPacketData > 0) {
                ...
                PACKET_HEADER *pHeader =
                    (PACKET_HEADER *)&m_PacketBuffer[nReadData];
    ...
    void ProcessPacket(const char *pData) {
        ...
        case NOTICE_CHAT: {
            ...
            std::cout << pPacket->szName << ": " << pPacket->szMessage
                      << std::endl;
        } break;
        }
    }
...
int main() {
    ...
    while (std::cin.getline(szInputMessage, MAX_MESSAGE_LEN)) {
        ...
        if (Cliet.IsLogin() == false) {
            PKT_REQ_IN SendPkt;
            SendPkt.Init();
            strncpy_s(SendPkt.szName, MAX_NAME_LEN, szInputMessage,
                      MAX_NAME_LEN - 1);
            ...
            SendPkt.Init();
            strncpy_s(SendPkt.szMessage, MAX_MESSAGE_LEN, szInputMessage,
                      MAX_MESSAGE_LEN - 1);
